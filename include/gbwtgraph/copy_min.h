...
//------------------------------------------------------------------------------
  ...
  void swap(MinimizerIndex& another)
  {
    ...
    this->hash_table_rymer.swap(another.hash_table_rymer);
  }
  ...
  MinimizerIndex& operator=(MinimizerIndex&& source)
  {
    if(&source != this)
    {
      ...
      this->hash_table_rymer = std::move(source.hash_table_rymer);
    }
    return *this;
  }
...
// Serialize the index to the ostream. Returns the number of bytes written and
// true if the serialization was successful.
std::pair<size_t, bool> serialize_rymer(std::ostream& out) const
{
  size_t bytes = 0;
  bool ok = true;

  bytes += io::serialize(out, this->header, ok);
  bytes += io::serialize_hash_table(out, this->rymer_hash_table, empty_hit(), ok);

  // Serialize the occurrence lists.
  for(size_t i = 0; i < this->capacity(); i++)
  {
    if(this->rymer_hash_table[i].first.is_pointer())
    {
      bytes += io::serialize_vector(out, *(this->rymer_hash_table[i].second.pointer), ok);
    }
  }

  if(!ok)
  {
    std::cerr << "MinimizerIndex::serialize_rymer(): Serialization failed" << std::endl;
  }
...
// Load the index from the istream and return true if successful.
bool deserialize_rymer(std::istream& in)
{
  bool ok = true;
  ...
  if(this->header.key_bits() != KeyType::KEY_BITS)
  {
    std::cerr << "MinimizerIndex::deserialize_rymer(): Expected " << KeyType::KEY_BITS << "-bit keys, got " << this->header.key_bits() << "-bit keys" << std::endl;
    return false;
  }
  
  this->header.update_version(KeyType::KEY_BITS);

  // Load the rymer_hash_table.
  if(ok) { ok &= io::load_vector(in, this->rymer_hash_table); }
  ...
  if(ok)
  {
    for(size_t i = 0; i < this->capacity(); i++)
    {
      if(this->rymer_hash_table[i].first.is_pointer())
      {
        this->rymer_hash_table[i].second.pointer = new std::vector<hit_type>();
        ok &= io::load_vector(in, *(this->rymer_hash_table[i].second.pointer));
      }
    }
  }
  ...
  if(!ok)
  {
    std::cerr << "MinimizerIndex::deserialize_rymer(): Index loading failed" << std::endl;
  }
...
{
    std::cerr << "[DEBUG] Starting minimizers function. Sequence length: " << (end - begin) << std::endl;
...
{
    std::cerr << "[DEBUG] Starting rymers function. Sequence length: " << (end - begin) << std::endl;
...
/*
    ...
    for (const auto& rymer_tuple : rymers_found) {
        std::cerr << " Rymer Key: " << std::get<0>(rymer_tuple).key
                  ...
                  << " Length: " << std::get<2>(rymer_tuple) << "\n";
    }

    // Print out details of minimizers
    std::cerr << "=== Minimizers Found ===\n";
    for (const auto& minimizer_tuple : minimizers_found) {
        std::cerr << " Minimizer Key: " << std::get<0>(minimizer_tuple).key
                  << " Decoded sequence: " << std::get<0>(minimizer_tuple).key.decode(this->k())
                  << " Start: " << std::get<1>(minimizer_tuple)
                  << " Hash: " << std::get<0>(minimizer_tuple).hash
                  << " Length: " << std::get<2>(minimizer_tuple) << "\n";
    }
...
*/
    ...
    std::unordered_multimap<size_t, std::tuple<minimizer_type, size_t, size_t>> minimizer_map;

    // Populate map using only the start value as the key
    for (const auto& minimizer_tuple : minimizers_found) {
        minimizer_map.emplace(std::get<1>(minimizer_tuple), minimizer_tuple);
    }
    ...
    // Lookup using only the start value
    for (const auto& rymer_tuple : rymers_found) {
...
//------------------------------------------------------------------------------
  ...
  */
  ...
  void insert_rymer(const minimizer_type& minimizer, code_type value, payload_type payload = DEFAULT_PAYLOAD)
  {
    ...
    {
      this->append_rymer({ value, payload }, offset);
    }
  }
...
std::pair<size_t, const hit_type*> count_and_find(const minimizer_type& minimizer) const
{
  ...
  //std::cerr << "Minimizer key: " << minimizer.key << std::endl;

  std::pair<size_t, const hit_type*> result(0, nullptr);

  if(minimizer.empty()) {
    std::cerr << "Minimizer is empty, returning result with no hits.\n";
    return result;
  }
  ...
  if(this->hash_table[offset].first == minimizer.key)
  {
    ...
    {
      result.first = 1;
      result.second = &(cell.second.value);
    }
  }
  else
  {
...
std::pair<size_t, const hit_type*> count_and_find_rymer(const minimizer_type& rymer) const {
    ...
    std::pair<size_t, const hit_type*> result(0, nullptr);
    if(rymer.empty()) {
        std::cerr << "count_and_find_rymer: Rymer is empty\n";
        return result;
    }
 ...
 void append_rymer(hit_type hit, size_t offset)
{
  ...
  cell_type& cell = this->hash_table_rymer[offset];
  if(cell.first.is_pointer())
  {
    std::vector<hit_type>* occs = cell.second.pointer;
    occs->push_back(hit);
